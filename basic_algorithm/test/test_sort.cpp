...
TEST_CASE("クイックソート", "[assert_sorted]") {
  std::vector<int> vec_empty{};
  std::vector<int> vec_one{12};
  std::vector<int> vec_two{2, 1};
  std::vector<int> vec_ten{11, 15, 13, 1, 234, 22, 3, 4, 13, 0};
  ...
  SECTION("test bubble sort") {
    ...
    // 要素数10の配列・昇順
    CHECK(quick_sorted(vec_ten, true) == std::vector<int>({0, 1, 3, 4, 11, 13, 13, 15, 22, 234}));
    // 要素数10の配列・降順
    CHECK(quick_sorted(vec_ten, false) == std::vector<int>({234, 22, 15, 13, 13, 11, 4, 3, 1, 0}));
  }
}